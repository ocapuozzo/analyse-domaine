...
Après avoir identifier des concepts métier clés, ces derniers seront **regroupés** (en référence au concept d'_encapsulation_ et de _classe_) et placés
...
Sur la base d'une petite étude de cas improvisée avec le concours
...
Les caractéristiques des magasins et des sociétaires ont été exportées du PGI et importées dans la nouvelle base de données utilisée par un outil de suivi. Votre cheffe de projet vous demande de faire évoluer la structure de la base de données pour qu'elle intègre les données nécessaires au suivi des engagements sociétaires.
