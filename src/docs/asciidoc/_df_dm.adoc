...
====
*Principe 1 -  la DF prime sur la DM (seules les DMR sont retenues)*
...
à une date donnée, à la demande d'un département de l'entreprise (id, nom, budget).
Botez que ce genre de demande (installation et mise à disposition d'un logiciel) ne peut être honorée que si le dit département ne dispose pas déjà d'une version
de ce même logiciel sur un des serveurs de l'entreprise.
...
On entend par _forme normale_ (en anglais _normal form_), un système de règles qui
qui permet de caractériser un modèle, selon une terminologie numérotée (1FN, 2FN, 3FN, ... 6FN)

Les formes normales ont été introduites par Edgar F. Codd (modèle relationnel).

Il est courant de dire qu'une base de données relationnelle est *_normalisée_* si elle est au minimum en troisième
forme normale. Un tel niveau de normalisation prémunie le modèle d'anomalies d'insertion, de mise à jour
...
(minimum 3FN) est très souvent l'objet du premier audit. Le but étant de *vérifier la robustesse de la conception*, l'absence de
de redondance et les problèmes sous-jacents.
...
Concerne les DMR. En prenant soin de concevoir (et de ne retenir) que des dépendances
directes et élémentaires (et réciproques pour les DM), et de vous assurer que les seules
dépendances de la relation sont celles qui lient la clé avec les autres attributs,
vous vous assurez que votre modèle sera en 4FN.
