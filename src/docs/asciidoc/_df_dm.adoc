...
Ce chapitre introduit 2 règles à suivre lorsqu'on analyse des données en vue de modéliser le domaine métier.
...
Toute source multi-attributs d'une DF (DF à partie gauche composée) se doit d'être une
DMR élémentaire et directe (prévisage d'une relation type many-to-many).
