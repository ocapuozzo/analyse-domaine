...
====
*Principe 2 - DF Multi-Attributs == DMR*
...
qui spécifie la contrainte associée à cette entité qui aurait pu être exprimée sous la forme d'une simple *contrainte d'unicité*.
...
V : non, par exemple cette vis nous est proposée par Grosdétail à 0,12 € et 0,35 € chez BigSA.
...
Une DMR porteuse d'information (impélemntation "scolaire")

image:diagUML-analyse-sr-ManyToMany-Porteuse.png[title="ManyToMany"]

=== ManyToMany porteuse (réification)

Absence de clé primaire composée : clé technique mono-attribut et clé candidate multi-attributs.
